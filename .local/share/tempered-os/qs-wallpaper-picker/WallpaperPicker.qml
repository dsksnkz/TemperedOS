...
Item {
    ...
    function applyDownloadedOnlineWallpaper(safeFileName) {
        ...
        const applyScript = `
            (
                ...
                pkill mpvpaper || true
                "$HOME/.local/bin/tempered-wallpaper-apply" "$DEST_FILE"
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (isVideo) {
            wallpaperCmd = `
                "$HOME/.local/bin/tempered-wallpaper-apply" "$WALL_FILE" video "$THUMB_FILE"
            `
            lockBgCmd = `cp "$THUMB_FILE" /tmp/lock_bg.png`
        } else {
            wallpaperCmd = `
                "$HOME/.local/bin/tempered-wallpaper-apply" "$WALL_FILE"
            `
